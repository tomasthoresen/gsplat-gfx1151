...
__device__ void dpp_sclr_warpSum(T &val) {
	...
	// val = __shfl(tmp, 63);
    rocprim_warpSum<GSPLAT_WARP_SIZE>(val, NULL);
}
...
__device__ void dpp_sprd_warpSum(T &val, int ln, T &n_val) {
	...
	//        n_val = tmp;
    T tmp = val;
    rocprim_warpSum<GSPLAT_WARP_SIZE>(tmp, NULL);
    if (cg::this_thread_block().thread_rank() == ln)
        n_val = tmp;
}
...
void launch_rasterize_to_pixels_3dgs_bwd_kernel(
    ...
    // The bs64 kernel assumes its 64-thread block is exactly one wavefront:
    // it uses the block thread rank as a lane id, 64-lane __shfl, and e%64 /
    // e/64 index arithmetic. That only holds on wave64 parts. On wave32 the
    // block spans two waves, so fall back to the generic kernel.
    const bool use_bs64 = (block_size == 64) && (GSPLAT_WARP_SIZE == 64);
    ...
    if (use_bs64) { // wave64-optimized path
      ...
      const uint32_t warps_per_block = (block_size + GSPLAT_WARP_SIZE - 1) / GSPLAT_WARP_SIZE; // for 64-lane warp
