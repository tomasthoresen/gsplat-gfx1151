...
#pragma unroll
            ...
            rocprim_warpSum<3, GSPLAT_WARP_SIZE>(v_normal_local, warp_storage_base);   // 3-sized float array
            rocprim_warpSum<GSPLAT_WARP_SIZE>(v_xy_local, warp_storage_base);          // vec2
            rocprim_warpSum<GSPLAT_WARP_SIZE>(v_u_M_local, warp_storage_base);         // float
            rocprim_warpSum<GSPLAT_WARP_SIZE>(v_v_M_local, warp_storage_base);         // float
            rocprim_warpSum<GSPLAT_WARP_SIZE>(v_w_M_local, warp_storage_base);         // float
            if (v_means2d_abs != nullptr) {
...
void launch_rasterize_to_pixels_2dgs_bwd_kernel(
    ...
    const uint32_t warps_per_block =
        (block_size + GSPLAT_WARP_SIZE - 1) / GSPLAT_WARP_SIZE;                       // for 64-lane warp
